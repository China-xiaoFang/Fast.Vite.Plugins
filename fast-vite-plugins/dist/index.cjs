...
`,c+=`	${s.iconName},`,m+=`export * from "./${s.iconName}";
`,r+1<i.length&&(c+=`
...
`)}}}var N=g(require("fs"),1),j=g(require("path"),1),I=g(require("rollup-plugin-external-globals"),1),E=require("vite-plugin-externals"),H=process.env.NODE_ENV==="development";function W(t){let e=process.cwd(),n=j.default.join(e,"node_modules",t,"package.json");return N.default.existsSync(n)?JSON.parse(N.default.readFileSync(n,"utf8")).version:""}function k(t){return!!(t.startsWith("http:")||t.startsWith("https:")||t.startsWith("//"))}function D(t,e){let{path:n}=e;return k(n)&&(t=n),t.replace(/\{name\}/g,e.name).replace(/\{version\}/g,e.version).replace(/\{path\}/g,n)}function _(t,e){e=t.prodUrl||e;let n=t,i=t.version||W(n.name),o=[];Array.isArray(n.path)?o=n.path:o.push(n.path);let p={...n,version:i};o=o.map(s=>{if(!i&&!k(s))throw new Error(`modules: ${p.name} package.json file does not exist`);return D(e,{name:p.name,version:p.version,path:s})});let c=n.css||[];!Array.isArray(c)&&c&&(c=[c]);let m=Array.isArray(c)?c.map(s=>D(e,{name:p.name,version:p.version,path:s})):[];return{...n,version:i,pathList:o,cssList:m}}function z(t){let{modules:e=[],prodUrl:n="https://cdn.jsdelivr.net/npm/{name}@{version}/{path}",enableInDevMode:i=!1,generateCssLinkTag:o,generateScriptTag:p}=t,c=!1,m=(Array.isArray(e)?e:[e]).map(a=>typeof a=="function"?a(n):a).map(a=>_(a,n)),s={};m.forEach(a=>{s[a.name]=a.var,Array.isArray(a.alias)&&a.alias.forEach(l=>{s[l]=a.var})});let r=[{name:"fast-vite-plugin-cdn-import",enforce:"pre",config(a,{command:l}){c=l==="build";let u={build:{rollupOptions:{plugins:[]}}};return c&&(u.build.rollupOptions.plugins=[(0,I.default)(s)]),u},transformIndexHtml(a){if(!c&&!i)return a;let l=[];return m.forEach(u=>{u.pathList.forEach(f=>{let C=p?.(u.name,f)||{},S={src:f,crossorigin:"anonymous",...u?.attrs??{},...C.attrs};l.push({tag:"script",attrs:S,...C})}),u.cssList.forEach(f=>{let C=o?.(u.name,f)||{},S={href:f,rel:"stylesheet",crossorigin:"anonymous",...u?.attrs??{},...C.attrs};l.push({tag:"link",attrs:S,...C})})}),l}}];return H&&i&&r.push((0,E.viteExternalsPlugin)(s,{enforce:"pre"})),r}var x=g(require("fs"),1),P=g(require("path"),1);function V(t){if(t)return{name:"fast-vite-plugin-tsx-component-auto-import",buildStart(){let e=typeof t=="string"?t:t.dir,n=r=>r.charAt(0).toUpperCase()+r.slice(1),i=typeof t=="string"?n:t.formatter??n,o=x.default.readdirSync(e,{withFileTypes:!0});if(o?.length===0)return;let p=o.filter(r=>r.name!=="index.ts").map(r=>({componentName:i(r.name),fileName:r.name})).sort((r,a)=>r.componentName<a.componentName?-1:r.componentName>a.componentName?1:0),c="",m="",s="";p.forEach(({componentName:r,fileName:a},l)=>{c+=`import { ${r} } from "./${a}";
`,m+=`export * from "./${a}";
`,s+=`	${r},`,l+1<p.length&&(s+=`
